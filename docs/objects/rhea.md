...
`Rhea` is a `CelestialBody` (Moon) in the solar-system registry (`SolarSystem`), parent `"saturn"`. It draws using the single shared UV-sphere `Mesh` uploaded once at startup — see [uv-sphere.md](uv-sphere.md) for how that mesh's vertices and triangles are built. This document covers only what's specific to `Rhea`: its transform, its real physical data, and its texture.

## Geometry generation

Not object-specific. `Rhea` contributes zero unique vertices or triangles — it reuses the one `Mesh` shared by all 26 bodies in the scene (bible failure mode F9: never upload a mesh per body). Full derivation of that shared sphere (stacks/slices grid, pole handling, seam duplication, index winding) is in [uv-sphere.md](uv-sphere.md).

## Vertex attributes / triangle construction

Identical to every other body in the scene — see [uv-sphere.md](uv-sphere.md) and [phase-2-rendering-pipeline.md](phase-2-rendering-pipeline.md). `Rhea` does not alter the `Vertex{position, normal, uv}` layout or the index buffer.

## Transform

- Class: `CelestialBody`, registry id `"rhea"`, parent `"saturn"`
- Local scale: `0.2358` (uniform, all axes) — real radius 763.8 km compressed the same way, THEN DIVIDED by saturn's own render radius — required because this is a scene-graph child and saturn's `worldMatrix` will multiply this local scale by saturn's own scale again; skipping that division is the exact bug [scale-manager.md](scale-manager.md) documents (a moon rendering ~12x too close and ~8x too small, mostly buried inside its planet).
- Local position: `(-1.1964, 0, 2.0723)` — initial orbit position (moves every frame — see Orbit above)
...
Elliptical orbit (`CelestialBody::setOrbit`, see [orbital-motion.md](orbital-motion.md)): semi-major axis `2.3917` render units, eccentricity `0.001`, angular velocity `0.5563` rad/s, initial true anomaly `120` degrees, orbit center `(0.0, 0.0, 0.0)` (local (relative to its parent) coordinates). At start, `r = a(1-e^2)/(1+e*cos(angle0))` = `2.3929`, giving position `(-1.1964, 0, 2.0723)`. This point moves every frame — it is the position *at startup*, not a fixed value.
...
- Local equator vertex `(1, 0, 0)` -> scale by `0.2358` -> `(0.2358, 0, 0)` -> rotate `0` degrees about +Z -> `(0.2358, 0, 0)` -> translate by local position -> world `(-0.9606, 0, 2.0723)`
- Local "north pole" vertex `(0, 1, 0)` -> scale -> `(0, 0.2358, 0)` -> rotate -> `(0, 0.2358, 0)` -> translate -> world `(-1.1964, 0.2358, 2.0723)`
...
- Rotation period: 108.4 hours
- Axial tilt: 0 degrees

## Material mapping

- Texture file: `assets/textures/bodies/rhea.jpg` (1440x720, loaded via `Texture2D::loadFromFile` → `stb_image` decode → `glTexImage2D`)
...
- UV coordinates come entirely from the shared sphere generator (`u = theta/2*pi`, `v = phi/pi`), computed independently of which texture file is bound — swapping `Rhea`'s texture changes zero vertex data, which is the proof that texturing and geometry are decoupled pipeline stages (see [phase-2-rendering-pipeline.md](phase-2-rendering-pipeline.md)).
...
- Startup `[ASSET]` log line confirms `assets/textures/bodies/rhea.jpg` loaded and its pixel resolution.
- Visual: `Rhea` renders as a lit, textured sphere, visibly moving along its orbit over time; toggling `setVisible(false)` on it hides only this body.
