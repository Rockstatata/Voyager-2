...
**Current state:** all bible phases are complete. 26 textured bodies share one 32×64 UV sphere. Planets and Voyager 2 are evaluated from dense Horizons state vectors (`MissionEphemeris`) at the single `SimulationClock` Julian Date; flybys match NASA's closest approaches. Voyager 2 is procedural (`VoyagerModelBuilder`) with Historical and 6-DOF Manual flight. Lighting has five techniques (Flat, Gouraud, Phong, Blinn-Phong, Toon), Sun point, headlamp spot and fill directional lights, derived normal/specular maps and ray-traced soft shadows; F9 is a Whitted ray-traced view including Voyager's triangles via `TriangleBvh`. Voyager is textured from NASA's public-domain atlas and has an Inspect mode over 18 components. Glow, translucent rings, floating origin, logarithmic depth, HUD/labels (`TextRenderer`, project-authored `BitmapFont`) and full camera autonomy (click-to-select, planet/moon stepping) are in place. `docs/guide/README.md` is the ground-up learning guide; `docs/objects/README.md` indexes how each piece works.
...
The working directory must be the repository root because shaders and textures use relative paths. Output is `x64\<Configuration>\Voyager-2.exe`. Verify changes with the relevant tests in bible section 43, run `scripts\verify_scene_layout.ps1` and `scripts\verify_navigation_and_motion.ps1`, and inspect the rendered result: `Voyager-2.exe --capture <dir>` (or `--capture-bodies`, `--capture-shading`, `--capture-raytrace`, `--capture-voyager`) writes a screenshot tour and exits. `scripts\fetch_horizons.ps1` regenerates the ephemeris CSVs; the program itself stays offline. When adding a `.cpp` or `.h`, register it in both `Voyager-2.vcxproj` and `Voyager-2.vcxproj.filters`, then rebuild.
...
`Vertex` is eight tightly packed floats: `position` at location 0, `normal` at location 1, and `texCoord` at location 2. `Mesh` and `shaders/scene.vert` must change together if this format changes. Shaders live in `shaders/` and share code through `#include`: `lighting.glsl` (`evaluateLights`, the only light model, used by raster and ray tracer), `raytrace.glsl` and `raytrace_mesh.glsl` (spheres, rings, shadow rays, Voyager's BVH). Per-draw uniforms come from `Material`: `baseColor`, `useTexture`/`albedoTexture` (unit 0), `uvTransform`, `shadingModel`, `specularStrength`, `specularPower`, `opacity`, `normalMap` (unit 1), `specularMap` (unit 2), `normalStrength`, `selfShadowing`. Per-frame: `model` (camera-relative), `view` (eye at origin), `proj`, `logDepthCoefficient`, `lights[4]`, `shadingTechnique`, `ambientStrength`, `lightingEnabled`, `surfaceMapsEnabled`, the trace scene (`spheres`, `rings`, `sunCenter`, `sunLightRadius`, `shadowMode`) and BVH buffers (units 3-5). Every look change goes through `Material` fields, never per-object shader code; see `docs/objects/lighting.md` and `docs/objects/ray-tracing.md`.
