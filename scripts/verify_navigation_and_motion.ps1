...
$applicationHeader = Read-Source 'src\core\Application.h'
...
Require ($application -match 'updateEphemerisPositions' -and $application -match 'planetRenderPosition') 'Planets are not placed from the shared dated ephemeris.'
Require ($application -match 'voyagerRenderPosition\(julianDate\)') 'Historical Voyager does not read the shared date.'
...
Require ($application -match 'nearestSurfaceDistance') 'Free-flight speed does not adapt to the nearest surface.'
Require ($camera -match 'kTransitionSeconds') 'Focus has no smooth fly-to transition.'
Require ($application -match 'movementKey && !pilotingVoyager') 'Fly keys do not hand locked views to free flight.'
Require ($applicationHeader -match 'm_trajectoryVisible = false') 'The long Voyager trajectory is shown by default.'
