...
- Shading: `Lit`, `ocean` preset: specular strength 0.55, power 48, ambient 0.07, a derived normal map (relief 1.2) and an ocean specular mask, so only water glints. Lit by the Sun point light (plus the optional headlamp and fill) in whichever technique `F3` selects (default Blinn-Phong) — [lighting.md](lighting.md). `K` toggles lighting off to show the raw texture; `F8` toggles the maps.
- Shadows: every fragment traces a soft shadow ray to the Sun through all body spheres and ring bands, so this body both casts and receives eclipse shadows (`F4`: off, hard, soft). In the ray-traced view (`F9`) it is an exact analytic sphere with mirror reflectivity 0.12 — [ray-tracing.md](ray-tracing.md).
