...
`Callisto` is a `CelestialBody` (Moon) in the solar-system registry (`SolarSystem`), parent `"jupiter"`. It draws using the single shared UV-sphere `Mesh` uploaded once at startup — see [uv-sphere.md](uv-sphere.md) for how that mesh's vertices and triangles are built. This document covers only what's specific to `Callisto`: its transform, its real physical data, and its texture.

## Geometry generation

Not object-specific. `Callisto` contributes zero unique vertices or triangles — it reuses the one `Mesh` shared by all 26 bodies in the scene (bible failure mode F9: never upload a mesh per body). Full derivation of that shared sphere (stacks/slices grid, pole handling, seam duplication, index winding) is in [uv-sphere.md](uv-sphere.md).

## Vertex attributes / triangle construction

Identical to every other body in the scene — see [uv-sphere.md](uv-sphere.md) and [phase-2-rendering-pipeline.md](phase-2-rendering-pipeline.md). `Callisto` does not alter the `Vertex{position, normal, uv}` layout or the index buffer.

## Transform

- Class: `CelestialBody`, registry id `"callisto"`, parent `"jupiter"`
- Local scale: `0.3255` (uniform, all axes) — real radius 2,410.3 km compressed the same way, THEN DIVIDED by jupiter's own render radius — required because this is a scene-graph child and jupiter's `worldMatrix` will multiply this local scale by jupiter's own scale again; skipping that division is the exact bug [scale-manager.md](scale-manager.md) documents (a moon rendering ~12x too close and ~8x too small, mostly buried inside its planet).
- Local position: `(-2.8842, 0, 0)` — initial orbit position (moves every frame — see Orbit above)
...
Elliptical orbit (`CelestialBody::setOrbit`, see [orbital-motion.md](orbital-motion.md)): semi-major axis `2.863` render units, eccentricity `0.0074`, angular velocity `0.1506` rad/s, initial true anomaly `180` degrees, orbit center `(0.0, 0.0, 0.0)` (local (relative to its parent) coordinates). At start, `r = a(1-e^2)/(1+e*cos(angle0))` = `2.8842`, giving position `(-2.8842, 0, 0)`. This point moves every frame — it is the position *at startup*, not a fixed value.
...
- Local equator vertex `(1, 0, 0)` -> scale by `0.3255` -> `(0.3255, 0, 0)` -> rotate `0` degrees about +Z -> `(0.3255, 0, 0)` -> translate by local position -> world `(-2.5587, 0, 0)`
- Local "north pole" vertex `(0, 1, 0)` -> scale -> `(0, 0.3255, 0)` -> rotate -> `(0, 0.3255, 0)` -> translate -> world `(-2.8842, 0.3255, 0)`
...
- Texture file: `assets/textures/bodies/callisto.jpg` (1440x720, loaded via `Texture2D::loadFromFile` → `stb_image` decode → `glTexImage2D`)
...
- UV coordinates come entirely from the shared sphere generator (`u = theta/2*pi`, `v = phi/pi`), computed independently of which texture file is bound — swapping `Callisto`'s texture changes zero vertex data, which is the proof that texturing and geometry are decoupled pipeline stages (see [phase-2-rendering-pipeline.md](phase-2-rendering-pipeline.md)).
...
- Startup `[ASSET]` log line confirms `assets/textures/bodies/callisto.jpg` loaded and its pixel resolution.
- Visual: `Callisto` renders as a lit, textured sphere, visibly moving along its orbit over time; toggling `setVisible(false)` on it hides only this body.
