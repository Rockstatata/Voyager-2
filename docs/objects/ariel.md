...
![Runtime Focus view of Ariel](images/bodies/ariel.jpg)

*Annotated runtime capture: `Tab` Focus view of Ariel at launch date 1977-08-21, Sun lighting on, labels and HUD visible (`Voyager-2.exe --capture-bodies <dir>`).*

## Overview

`Ariel` is a `CelestialBody` (Moon) registered by `SolarSystem` as parent `"uranus"`. It draws with the one shared 32x64 UV-sphere `Mesh` ([uv-sphere.md](uv-sphere.md)); this page covers only what is specific to Ariel: scale, motion, physical data, material and verification.

## Geometry generation

None of its own. Ariel contributes zero unique vertices: all 26 bodies share one sphere upload (2,145 vertices, 3,968 triangles, bible F9). The derivation of every vertex, UV, index and the duplicated seam column is in [uv-sphere.md](uv-sphere.md).
...
- World radius: `0.1186` = 0.50 x (578.9 / 6,371)^0.60; Uranus's is `1.1454`.
- Local scale: `0.1035` = world radius / Uranus radius, because the parent's world matrix multiplies it by Uranus's scale again ([scale-manager.md](scale-manager.md), parent-scale compensation).
...
- semi-major axis `4.3435` parent radii = 1.6 + sqrt(190,900 / 25,362.0) (`ScaleManager::moonOrbitDistanceToRenderUnits`), eccentricity `0.0012`;
- angular rate `0.9973` rad/s = 2 pi x 0.4 / 2.52 days (moon visual clock: 0.4 days per second at 1x);
- starting true anomaly `137.5` deg (sibling 1 x golden angle 137.5 deg), giving local position `(-3.2056, 0, 2.9366)`.

## Worked vertex example

Local equator vertex `(1, 0, 0)` -> scale `0.1035` -> tilt 0.0 deg about +Z -> `(0.1035, 0.0000, 0)` (spin 0) -> plus the body position. That sum is still in Uranus's local frame; the parent world matrix (its tilt, scale and position) is applied next. Every matrix is double precision until `Renderer::submit` subtracts the camera position and narrows to float.
...
- Visual: `Tab` until the HUD shows `CAMERA FOCUS: ARIEL`; the camera flies in on the day side and the label reads `ARIEL`. Wheel zooms to the surface, drag orbits, `W` leaves into free flight.
